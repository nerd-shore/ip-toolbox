...
export const isMinDate = (props, propName, componentName) => {
  ...
  if (minDate) {
    if (!(minDate instanceof Date)) {
      return new Error(
        `Invalid prop "${propName}" of type "${typeof minDate}" supplied to "${componentName}",
...
export const isMaxDate = (props, propName, componentName) => {
  ...
  if (maxDate) {
    if (!(maxDate instanceof Date)) {
      return new Error(
        `Invalid prop "${propName}" of type "${typeof maxDate}" supplied to "${componentName}", 
        expected instance of "Date".`
      );
    }
...
export const isView = (props, propName, componentName) => {
  ...
  if (allowedViews.indexOf(view) === -1) {
    return new Error(
      `Invalid prop "${propName}" of value "${view}" supplied to "${componentName}", 
      expected one of [${['a', 'b', 'c', 'd', 'e'].map(a => `"${a}"`).join(', ')}].`
    );
  }
...
isView = (props, propName, componentName) => {
  const view = props[propName];
  if (!view) {
